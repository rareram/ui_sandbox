...
import { Database, Edit, Trash2, Search, Plus } from 'lucide-react';
...
const SourceList: React.FC = () => {
  ...
  return (
    <div className="p-6 h-full flex flex-col">
       {/* Actions Bar (Search & Add) - Title removed */}
      ...
      <div className="flex-1 overflow-hidden bg-white dark:bg-slate-900 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col">
        ...
        <div className="overflow-y-auto custom-scrollbar flex-1">
            {filtered.map((source) => (
                <div key={source.id} className="grid grid-cols-12 gap-4 p-4 items-center border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                    <div className="col-span-3 flex items-center gap-3">
                        <div className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                            <Database size={18} />
                        </div>
                        <div>
                            <p className="font-bold text-slate-800 dark:text-slate-200">{source.name}</p>
                            <p className="text-xs text-slate-500 font-mono">{source.ip}</p>
                        </div>
                    </div>
                    <div className="col-span-2 text-sm text-slate-600 dark:text-slate-400">
                        <span className="bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded text-xs font-bold">{source.type}</span>
                    </div>
                    ...
                    <div className="col-span-1">
                         <span className={`px-2 py-1 rounded text-xs font-bold ${
                            source.status === 'CONNECTED' ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400' : 
